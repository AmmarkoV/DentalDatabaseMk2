...
if Ioresult=0 then
  begin
   writeln(fileused,'<html><head><title>��������� ��������</title><meta http-equiv="Content-Type" content="text/html; charset=windows-1253"></head>');
...
writeln(fileused,'��������� ���������� �������� '+Convert2String(datesnstuff[1])+'/'+Convert2String(datesnstuff[3])+'/'+Convert2String(datesnstuff[4])+'</center>');
...
for i:=1 to alias_types_count do
   begin
     ...
     writeln(fileused,'<tr><td>�������</td><td>����� ��������</td><td>����</td><td>����.</td></tr>');
...
MessageBox (0, '������� OK ���� ���������� �� ��� ���������� ���������' , ' ', 0 + MB_ICONASTERISK);
  end;
end;
...
include_object('Window_Works_Select_A_Work','window','������� ��������','Works_Select_A_Work','','',(GetMaxX div 2)-350,000,(GetMaxX div 2)+350,200+(endfrom-startfrom+3)*space);
...
if ((startfrom>0)and(endfrom<=MAX_ALIASES)and(startfrom<endfrom)) then
     begin
      for i:=startfrom to endfrom do
       ...
       if ((alias_str[2,i]='') and (alias_int[2,i]=0) ) then retres:='����';
       include_object('Works_Select_BTN('+Convert2String(i),'buttonc',retres,'Works_Select_A_Work','','',-1,50+(i-startfrom)*space,-1,0);
       end;
     end; 
include_object('exit2','buttonc','������','Works_Select_A_Work','','',-1,Y2(last_object)+20,-1,0);
draw_all; 
repeat
interact;
...
flush_gui_memory(0);
space:=30;
include_object('Window_Works_Select_Work','window','������� ���������� ��������','Works_Select_Work','','',(GetMaxX div 2)-150,200,(GetMaxX div 2)+150,300+(alias_types_count+2)*space);
...
include_object('exit','buttonc','������','Works_Select_Work','','',-1,yexit-40,-1,0);
draw_all;
repeat
interact;
lastobj:=return_last_mouse_object;
...
if alias_types_count>category then
 ...
 MessageBox (0,pchar('� ������������ ������ ��� ����� ����������� ����� ��� ��� '+Convert2String(MAX_ALIASES)+' ��������.. '), '���� ��������', 0 + MB_ICONASTERISK);
...
startx:=GridX(1,3)-70;
starty:=200;
include_object('window1','window','������ ��o '+Convert2String(mem_spot),'no','','',startx,starty,GetMaxX-startx,starty+300);
draw_all;
...
include_object('comment_code','comment','�������','no','','',X1('code'),Y1('code')-22,0,0);
include_object('comment_strcode','comment','����������','no','','',X1('strcode'),Y1('strcode')-22,0,0);
include_object('comment_price','comment','����','no','','',X1('price'),Y1('price')-22,0,0);
include_object('comment_Ergasia','comment','���������','no','','',X1('description'),Y1('description')-22,0,0);

include_object('ok','buttonc','������','no','','',-1,starty+blocky*4,-1,0);
include_object('exit','buttonc','������','no','','',-1,starty+blocky*5,-1,0);
draw_all;
repeat
 interact;
 if get_object_data('ok')='4' then begin
                                    ...
                                    if ( (Get_Work_Mem_From_Code(get_object_data('code'))<>-1) and (Get_Work_Mem_From_Code(get_object_data('code'))<>mem_spot) ) then
                                     begin
                                      MessageBox (0, pchar('����� ������� ����� ������ ������� '+Convert2String(Get_Work_Mem_From_Code(get_object_data('code')))) , ' ', 0+ MB_ICONASTERISK + MB_SYSTEMMODAL);
                                     end else
                                    if z<>0 then
                                     begin  
                                      set_button('exit',1);
                                     ...
                                     MessageBox (0, '��� �������� �� ��������������� ��� ������ 0 , ����� �����������' , ' ', 0 + MB_ICONASTERISK + MB_SYSTEMMODAL);
                                  end;
 until GUI_Exit;
end;
...
start_gui_prices:
 ...
 starty:=200; 
 blocky:=30;
 include_object('window1','window','������ ���� �����','no','','',startx,starty,GetMaxX-startx,starty+230);
 draw_all;
 delete_object('window1','name');
 starty:=starty+40;
 include_object('new','buttonc','�������� ������� ����� �� ���������','no','','',-1,starty+blocky*0,-1,0);
 include_object('start','buttonc','������ �� ������ ����','no','','',-1,starty+blocky*1,-1,0);
 include_object('print','buttonc','�������� ��������� �����','no','','',-1,starty+blocky*2,-1,0);
 include_object('save','buttonc','����������','no','','',-1,starty+blocky*3,-1,0);
 include_object('exit','buttonc','������','no','','',-1,starty+blocky*4,-1,0);
 draw_all;
 repeat
  interact;
                                            ...
                                            if mem_spot<>0 then
                                                 begin
                                                   ...
                                                   price:=Get_Work_Int(mem_spot,2); 
                                                    ...
                                                    startx:=GridX(1,3)-70;
                                                    starty:=200; 
                                                    include_object('window1','window','������ ��o '+retres,'no','','',startx,starty,GetMaxX-startx,starty+300);
                                                    draw_all;
                                                    delete_object('window1','name');
                                                    starty:=starty+60;
                                                    include_object('code','textbox',Convert2String(the_code),'no','','',startx+20,starty,startx+tbox*5,0);
